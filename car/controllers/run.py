...
TARGETS_FILE = os.path.join(BASE_DIR, "entities", "target_points.json")
...
def _move_segment_monitored(x1, y1, x2, y2, cli,
    ...
    while True:
        ...
        if seg_len < POS_THRESHOLD:  # 剩余距离足够短，视为完成
        ...
        if result.get("yaw_interrupt"):
            # 监控中曾检测到偏角过大，不再信任重新读取的结果，直接校正
            print(f"  偏角过大（监控检测到），校正中...")
            cli.sync_yaw()
        else:
            yaw = _read_angle_deviation()
...
def _do_correction(cli, expected_x, expected_y):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if err <= POS_THRESHOLD:
            print(f"  {'─' * 70}")
            print(f"  ✓ 已收敛（误差 {err:.3f}m <= {POS_THRESHOLD:.3f}m）")
            break
...
def _do_target_correction(cli, pid, target_info):
    ...
    if abs(cdx) > POS_THRESHOLD or abs(cdy) > POS_THRESHOLD:
        ...
        print(f"        ✓ 已在精校阈值内（阈值 {POS_THRESHOLD:.3f}m）")
...
def _execute_point(num, targets, zones, start_pos, cli, obstacles=None):
    ...
    if num == 7:
        ...
        # 目标点6特殊逻辑：路径规划先到 x=3.60，再由 correct 动作微调至真实坐标，避免碰撞
        if num == 6:
            print(f"  目标点6 安全策略：先导航至 (3.60, {ty:.3f})，再微调至 ({tx:.3f}, {ty:.3f})")
    else:
        return False

    # 路径规划使用的目标坐标（点6特殊处理：先到安全接近点，再微调）
    if num == 6:
        plan_tx, plan_ty = 3.60, ty
    else:
        plan_tx, plan_ty = tx, ty
    ...
    print(f"\n{'─' * 40}")
    if num == 6:
        print(f"  {label}: 接近点({plan_tx:.2f}, {plan_ty:.3f}) → 真实({tx:.3f}, {ty:.3f})  当前: ({cx:.3f}, {cy:.3f})")
    else:
        print(f"  {label}: ({tx:.2f}, {ty:.2f})  当前: ({cx:.3f}, {cy:.3f})")

    if not (abs(cx - plan_tx) < POS_THRESHOLD and abs(cy - plan_ty) < POS_THRESHOLD):
        print(f"\n  [规划] {label} 路径规划中...")
        ...
        result = plan_path(cx, cy, plan_tx, plan_ty, cx, cy, forbidden_zones=zones,
                           obstacles=obstacles)
        wp = result["waypoints"]
        ...
        while seg_idx < len(wp) - 1:
            ...
            if need_replan:
                ...
                result = plan_path(cx, cy, plan_tx, plan_ty, cx, cy,
    ...
    for act in actions:
        ...
        if t == "correct":
            ...
            while True:
                choice = input("  >>> [l/r=平移±0.2, 数字=去往该点, Enter=完成] ").strip()
                if choice == "":
                    break
                elif choice.lower() == "l":
                    cli.move_relative(0, 0.2)
                    print(f"  平移: Y+0.2")
                elif choice.lower() == "r":
                    cli.move_relative(0, -0.2)
                    print(f"  平移: Y-0.2")
...
def _run_interactive(start_x, start_y, targets, zones, cli, obstacles):
    ...
    while True:
        ...
        try:
            ...
            print("无效输入")
            continue
        result = _execute_point(num, targets, zones, (start_x, start_y), cli, obstacles)
        # adjust_x 中用户输入了数字 → 直接跳转到对应点位
        while type(result) is int:
            result = _execute_point(result, targets, zones, (start_x, start_y), cli, obstacles)
...
def main():
    ...
    if not targets:
        print("未找到 target_points.json，请先运行 target_zone_setup.py 设置点位")
        return
