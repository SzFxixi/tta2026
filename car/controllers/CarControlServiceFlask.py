...
if __name__ == "__main__":
    ...
    def move():
        try:
            ...
            while(abs(float(location[0]) - (getx() - car.x_offset)) > cfg.server.move_tolerance and attempts < cfg.server.move_max_retries):
                move_x = (getx() - car.x_offset) - float(location[0])
                cmd = f"chassis move x {move_x} y 0 z 0;"
                car.Move(cmd)
                attempts += 1
            attempts = 0
            while(abs(float(location[1]) - (gety() - car.y_offset)) > cfg.server.move_tolerance and attempts < cfg.server.move_max_retries):
    ...
    def move_long_distance():
        try:
            ...
            attempts = 0
            while(abs(float(location[0]) - (getx() - car.x_offset)) > cfg.server.long_distance_tolerance and attempts < cfg.server.move_max_retries):
                move_x = (getx() - car.x_offset) - float(location[0])
                cmd = f"chassis move x {move_x} y 0 z 0;"
                car.Move(cmd)
                attempts += 1
            attempts = 0
            while(abs(float(location[1]) - (gety() - car.y_offset)) > cfg.server.long_distance_tolerance and attempts < cfg.server.move_max_retries):
                move_y = (gety() - car.y_offset) - float(location[1])
                cmd = f"chassis move x 0 y {move_y} z 0;"
                car.Move(cmd)
                attempts += 1
